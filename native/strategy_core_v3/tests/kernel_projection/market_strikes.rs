use super::*;
use strategy_core_v3::decision_v5::{DecisionV5Error, MarketStrikesV5, decision_fence_v5_sha256};
...
fn native_caps_are_scoped_unambiguous_and_fenced() {
    ...
    let fence = decision_fence_v5_sha256(&context).unwrap();
    let mut changed = context.clone();
    changed.market_strikes.as_mut().unwrap()[0].cap_strike_milli_f = Some(81_000);
    assert_ne!(decision_fence_v5_sha256(&changed).unwrap(), fence);
    for mutation in 0..4 {
        ...
        assert_eq!(invalid.validate(), Err(DecisionV5Error::InvalidContract));
    }
}
