//! Station time zones and NWS climate days.
//!
//! A climate day runs midnight to midnight in the station's *standard* time all year, so in
//! daylight time it ends at 01:00 local. Ported unchanged from the legacy contract.
